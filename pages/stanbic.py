...
def parse_stanbic_pdf(uploaded_file):
    """
    Parse all pages of a Stanbic KES bank statement PDF into a DataFrame.
    ...
    """
    # Amounts: digits+commas+decimals, NOT preceded by a letter/digit/comma
    # (prevents matching e.g. "08.00" inside "KES108.00")
    AMT_RE = re.compile(r'(?<![A-Za-z\d,])-?[\d,]+\.\d{2}')
    DATE_RE = re.compile(r'^\d{2}/\d{2}/\d{4}')
    # Lines that signal the end of the transaction section on a page
    ...
    # Group lines into per-transaction blocks across all pages in order
    all_blocks = []
    ...
    for block in all_blocks:
        ...
        # All text in the block for amount extraction
        full_text = rest + ' ' + ' '.join(block[1:])
        all_amounts = AMT_RE.findall(full_text)

        # Description = block text with amounts stripped out
        ...
        # Skip balance-summary rows but track the balance value
        ...
        # Need at least [transaction_amount, balance]
...
st.title("🏦 Stanbic Reconciliation (KES)")

# First file uploader with a unique key
bank_statement = st.file_uploader("⬆️ Upload Bank Statement (PDF)", type=[".pdf"], key="bank_statement")
...
if bank_statement:
    ...
    with st.expander("Below is the uploaded Bank Statement", expanded=False, icon="🔽"):
        ...
        bank_statement = parse_stanbic_pdf(bank_statement)
...
def reconciler(erp_file, bank_file, match_scale):
    if erp_file is not None and bank_file is not None:
        ...
        if (bank_file["Credit"] != 0).any() and (bank_file["Debit"] != 0).any():
            ...
            # Combine direct + extended matches, then deduplicate on VOUCHER_NO
            # (direct matches come first so they take priority over extended ones)
            ...
            # ---- Helper: format a BRS-column subset for display / download ----
            BRS_COLS = [
            ...
            # ---- Matched download ----
            matched_export = to_brs_export(final_matched)
            matched_xls = to_xls_bytes(matched_export, "Matched BRS")
...
if bank_statement is None or erp_transactions is None:
    ...
    # ---- Bank charges summary (575.00 debit rows) ----
    charges = bank_statement[bank_statement["Debit"] == 575.0]
    st.markdown("### 🏦 Bank Charges Summary")
    col1, col2 = st.columns(2)
    col1.metric("Number of Charge Transactions", len(charges))
    col2.metric("Total Bank Charges (KES)", f"{charges['Debit'].sum():,.2f}")
